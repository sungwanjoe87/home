...
for word in level:
    inwds = input('{}의 영단어를 입력하세요.(0.프로그램 종료)>>'.format(word))
    if inwds.isdigit():
        if inwds == 0:
            print('프로그램을 종료합니다.')
            break
    if level[word] == inwds:
        ...
        #저장 wordlist [[car, O, 자동자, car],[.....]]
        wordlist.append(wlist)

    else:
        #오답 -> 오답입니다. 자동차:car
        print('오답입니다. {} : {}'.format(word,level[word]))
        #오답 n힝목 : crr, X, 자동차, car #1차 리스트 형태로
...
for idx,wlist in enumerate(wordlist): #정답 오답이 모두 저장된 2차원 리스트에 색인 출력
    if 'O' in wlist: #wordlist안의 1차원 리스트 wlist에 O이 있는지 확인.
        ocount +=1
        print('{}.{}, {}:{}, '.format(idx+1,'정답',wlist[2],wlist[3]))

    else:
        xcount +=1
        print('{}.{}, {}:{}, 내가 적은 답:{}'.format(inx+1,'오답',wlist[2],wlist[3],wlist[0]))
